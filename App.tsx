        ...
        <div className="relative z-10 w-full h-full animate-fade-in">
          ...
          {!(process.env.GEMINI_API_KEY || process.env.API_KEY) && (
            <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[70] w-[90%] max-w-md">
              <div className="bg-rose-500 text-white px-4 py-3 rounded-2xl shadow-2xl flex items-center gap-3 border border-rose-400">
                <div className="bg-white/20 p-2 rounded-xl">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 15c-.77 1.333.192 3 1.732 3z" /></svg>
                </div>
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider">API Key Missing</p>
                  <p className="text-[10px] opacity-90">Please add GEMINI_API_KEY in Settings to load live news.</p>
                </div>
              </div>
            </div>
          )}
