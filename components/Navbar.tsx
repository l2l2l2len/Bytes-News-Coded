    ...
    <nav className="fixed bottom-8 left-0 right-0 z-[40] flex justify-center pointer-events-none">
      ...
      <div className="glass-dock px-8 py-3 rounded-full flex items-center gap-10 pointer-events-auto animate-enter">
        ...
        <button className="group relative flex flex-col items-center gap-1" aria-label="Home">
           <div className="p-2 rounded-xl group-hover:bg-indigo-50/50 transition-colors">
              <svg className="w-6 h-6 text-indigo-950" fill="currentColor" viewBox="0 0 24 24"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg>
           </div>
           {/* Active Dot */}
           <div className="w-1 h-1 bg-indigo-500 rounded-full shadow-[0_0_8px_rgba(99,102,241,0.5)]"></div>
        </button>

        {/* Search */}
        <button onClick={onSearchClick} className="group relative flex flex-col items-center gap-1 text-indigo-900/40 hover:text-indigo-950 transition-colors" aria-label="Search">
           <div className="p-2 rounded-xl group-hover:bg-indigo-50/50 transition-colors">
        ...
        {/* Notifications */}
        <button onClick={onNotifyClick} className="group relative flex flex-col items-center gap-1 text-indigo-900/40 hover:text-indigo-950 transition-colors" aria-label="Notifications">
           <div className="p-2 rounded-xl group-hover:bg-indigo-50/50 transition-colors relative">
             <div className="absolute top-2 right-2 w-2 h-2 bg-rose-500 rounded-full border border-white"></div>
        ...
        <button onClick={onProfileClick} className="group relative flex flex-col items-center gap-1" aria-label="Profile">
           <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 p-[2px] group-active:scale-95 transition-transform">
              <div className="w-full h-full rounded-full bg-white flex items-center justify-center">
                 <span className="font-bold text-indigo-950 text-xs">B</span>
              </div>
           </div>
           <div className="w-1 h-1 bg-transparent rounded-full"></div>
        </button>
