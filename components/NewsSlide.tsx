      ...
      <div className="relative z-10 w-full px-4 md:px-6 pb-24 md:pb-28 flex items-end justify-between gap-3 md:gap-4">
         <div className={`flex-1 min-w-0 transition-all duration-700 delay-100 ease-out transform ${inView ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
             <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/80 backdrop-blur-md rounded-full mb-2 shadow-lg">
                <div className="w-3.5 h-3.5 rounded-full bg-gradient-to-tr from-pink-400 to-rose-400 flex items-center justify-center text-[7px] text-white font-bold">B</div>
                 ...
                 <div className="flex flex-col gap-1.5 md:gap-2">
                     ...
                     <div className="flex items-center justify-between pt-2.5 mt-1 border-t border-[#831843]/10">
                        <div className="flex items-center gap-2 truncate">
                            {byte.sourceUrl && (
                                <img src={`https://www.google.com/s2/favicons?domain=${new URL(byte.sourceUrl).hostname}&sz=32`} className="w-3.5 h-3.5 md:w-4 md:h-4 rounded-full opacity-80" alt="" />
                            )}
                        ...
                        {isExpanded && (
                             <button 
                                ...
                                className="px-4 py-1.5 bg-[#831843] text-white rounded-full text-[9px] font-bold uppercase tracking-widest hover:scale-105 transition-transform shrink-0 shadow-lg active:opacity-70 flex items-center gap-2"
                             >
                                Full Story
         ...
         <div className="flex flex-col items-center gap-4 mb-1 shrink-0 w-10 md:w-12">
             <div className={`flex flex-col items-center gap-1 transition-all duration-500 delay-[200ms] ${inView ? 'translate-x-0 opacity-100' : 'translate-x-4 opacity-0'}`}>
                 <button onClick={() => onLike(byte.id)} className="p-1 transition-transform active:scale-75 group">
                     <div className="p-2.5 md:p-3 rounded-full bg-white/70 backdrop-blur-md shadow-lg group-hover:bg-white transition-all">
                        <svg className={`w-5 h-5 md:w-6 md:h-6 transition-all duration-300 ${byte.isLiked ? 'fill-rose-500 text-rose-500 scale-110' : 'fill-white text-[#831843] group-hover:scale-110'}`} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={byte.isLiked ? 0 : 2}><path strokeLinecap="round" strokeLinejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
                     </div>
                 </button>
                 <span className="text-white text-[9px] md:text-[10px] font-bold drop-shadow-md">{byte.likes}</span>
             </div>
             ...
             <div className={`flex flex-col items-center gap-1 transition-all duration-500 delay-[400ms] ${inView ? 'translate-x-0 opacity-100' : 'translate-x-4 opacity-0'}`}>
                 <button onClick={() => onSave(byte.id)} className="p-1 transition-transform active:scale-75 group">
                    <div className="p-2.5 md:p-3 rounded-full bg-white/70 backdrop-blur-md shadow-lg group-hover:bg-white transition-all">
                        <svg className={`w-4 h-4 md:w-5 md:h-5 transition-all duration-300 ${byte.isSaved ? 'fill-[#831843] text-[#831843]' : 'fill-none text-[#831843] hover:scale-110'}`} stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z" /></svg>
                    </div>
                 </button>
                 <span className="text-white text-[9px] md:text-[10px] font-bold drop-shadow-md">Save</span>
             </div>
             ...
             <div className={`flex flex-col items-center gap-1 transition-all duration-500 delay-[500ms] ${inView ? 'translate-x-0 opacity-100' : 'translate-x-4 opacity-0'}`}>
                 <button onClick={handleShare} className="p-1 transition-transform active:scale-75 group">
                    <div className="p-2.5 md:p-3 rounded-full bg-white/70 backdrop-blur-md shadow-lg group-hover:bg-white transition-all">
                        <svg className="w-4 h-4 md:w-5 md:h-5 text-[#831843]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg>
                    </div>
                 </button>
                 <span className="text-white text-[9px] md:text-[10px] font-bold drop-shadow-md">Share</span>
             </div>
         </div>
      </div>
    </div>
